...
- Clock states: `running`, `closed` (terminating event occurred **on or before the due date** — an event recorded after the deadline does not close the clock, because a late award does not un-lapse proceedings that have already lapsed), `extended` (extension event with authority + reasons + new due date, and only on a clock the rule-set declares `extendable`; a `new_due_date` must be later than the current due date), `suspended` (court stay or statutory suspension event; resumes on vacate), `breached` (due date passed with no in-time terminating or extension event), `lapsed` (breach whose statutory consequence is lapse/rescission — case moves to `LAPSED`).
- Suspension: stay/vacate orders are paired in `occurred_at` order (a vacate closes the most recent open stay), each closed interval is clipped to the clock's own life so a stay that began before the clock started credits only from its start date, and overlapping intervals are unioned — two concurrent writs over the same weeks are counted once. A clock with any stay still open is `suspended` and scores no risk: nobody is in default while a court has stopped the proceeding.
- A statutory consequence (`on_breach`) is appended as the SYSTEM actor only where the rule-set permits that transition from the case's current stage. A consequence that is not available there is refused and raised as an `INTEGRITY` alert rather than forced through, which would leave a case in a terminal stage with no legal correction path.
...
- Preconditions are enforced: no `DECLARATION_S19` without `RR_SCHEME_PUBLISHED_S18` and `COST_DEPOSITED_S19_2`; no `POSSESSION_TAKEN_S38` while `compensation_paid < compensation_assessed` for the case unless an `URGENCY_S40` event exists with ≥80% paid *(verify)*. `compensation_assessed = 0` (no award has been valued) never satisfies the gate, and a recorded `COMPENSATION_PAID_FULL` does not satisfy it either — it is a marker that cannot be withdrawn by a later s.64 enhancement, so the paise decide.
