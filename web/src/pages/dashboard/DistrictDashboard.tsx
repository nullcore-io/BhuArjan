/**
 * District dashboard — Docs/Frontend.md §4, Docs/APIs.md §3.10, Docs/rules.md C7.
 *
 * `/national`, `/states/:state` and `/districts/:district` "share one layout"
 * (Frontend.md §4) and the API returns one shape for all three, so this screen is
 * deliberately the national one narrowed to a district: the same nine PS tiles,
 * the same three charts, the same risk queue — computed by the server over this
 * district's cases, not filtered client-side from a national payload. An officer
 * who has been shown the national screen already knows how to read this one.
 *
 * Two things are district-specific and matter:
 *  - the scope is a *string* the server resolves by name, LGD code or uuid, so an
 *    unresolvable one produces an empty-but-valid dashboard. Zeroes that mean
 *    "nothing matched" are called out as such rather than reported as facts;
 *  - `/dashboards/districts/{district}/explain` takes `kpi` only (no sector or
 *    statute), so the drawer forwards nothing else and says which scope it read.
 *
 * The layout is a copy of NationalDashboard.tsx rather than a shared component:
 * that file is frozen for this change. Factoring both onto one `<DashboardView>`
 * is the obvious follow-up and is listed as such.
 */
...
interface TileDef {
  ...
  /** KPI name accepted by the district explain endpoint, when explainable. */
  explain?: string
}
...
export default function DistrictDashboard() {
  ...
  const dash = useQuery({
    queryKey: ['dashboard', 'district', scope, sector, statute, from, to],
    queryFn: () => api<DashboardResponse>(`/dashboards/districts/${encoded}${suffix}`),
  ...
  const caseCount = num(kpis.case_count)
  ...
  // The alert list takes the same loose district parameter (Docs/APIs.md §3.9),
  // so the fallback stays inside this district rather than going national.
  const alertsFallback = useQuery({
    queryKey: ['alerts', 'districtFallback', scope],
    queryFn: () => api<unknown>(`/alerts?district=${encoded}&limit=10`),
    enabled: dash.isSuccess && risk.length === 0 && caseCount > 0,
    retry: false,
  })
  ...
  const scale = moneyScale(
    cumulative ? rawSeries.reduce((s, p) => s + num(p.assessed_paise), 0) : maxPaise,
  )
  ...
  function subFor(key: keyof Kpis): string | null {
    switch (key) {
      ...
      case 'families_affected':
        return caseCount ? `across ${formatCount(caseCount)} cases in this district` : null
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted">
            Every figure is this district's cases summed to{' '}
        ...
        <form className="flex flex-wrap items-end gap-2" onSubmit={(e) => e.preventDefault()}>
          ...
          <Link className="btn" to={`/reports?district=${encoded}`}>
            Export…
          </Link>
        </form>
      </header>
      ...
      {dash.isSuccess && caseCount === 0 ? (
        <div className="rounded border border-amber bg-amber/5 px-3 py-2 text-xs text-[#8A5300]">
          <strong className="font-semibold">No cases resolve to “{scope}”.</strong> The
          scope is matched server-side against the district's uuid, LGD code or name
          (Docs/APIs.md §3.10), and the caller's own jurisdiction is applied first —
          so this is either a district with no acquisition cases, a spelling that does
          not match the org-unit register, or a district outside your scope. The
          zeroes below are the absence of matching cases, not measured figures.
        </div>
      ) : null}

      {!scope ? (
        <Empty>No district was named in the URL. Open one from the national dashboard.</Empty>
      ) : null}
      {dash.isLoading ? <Loading label={`Loading ${scope || 'district'} figures…`} /> : null}
      {dash.isError ? <ErrorNote error={dash.error} what="District dashboard" /> : null}
...
function ExplainDrawer({
  ...
  // GET /dashboards/districts/{district}/explain takes `kpi` only — the sector and
  // statute filters the tiles honour are not part of that signature, so forwarding
  // them would produce an explanation that silently disagrees with the tile.
  const q = useQuery({
    queryKey: ['explain', 'district', district, kpi],
    queryFn: () =>
      api<ExplainResponse>(
        `/dashboards/districts/${district}/explain?kpi=${encodeURIComponent(kpi ?? '')}`,
      ),
    enabled: Boolean(kpi),
  })
  ...
  return (
    <Drawer
      ...
      subtitle={
        q.data
          ? `${scopeLabel} · as of seq ${q.data.as_of_seq ?? '—'} · ${eventIds.length} contributing events across ${cases.length} cases`
          : `Contributing events from the ledger — ${scopeLabel}`
      }
      onClose={onClose}
    >
